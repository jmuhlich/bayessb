...
def likelihood(mcmc, position):
    ...
    _, yobs = mcmc.options.model.simulate(mcmc.options.tspan, param_values,
                                          view=True)
...
def filter_params(model, prefix):
...
if __name__ == '__main__':
    ...
    _, yobs_initial = opts.model.simulate(opts.tspan, [p.value for p in opts.model.parameters])
    _, yobs_final = opts.model.simulate(opts.tspan, mcmc.cur_params(mcmc.position))
    plt.plot(opts.tspan, exp_ecrp, 'o', mec='red', mfc='none')
    plt.plot(opts.tspan, yobs_initial['CPARP_total']/parp_initial, 'b');
    plt.plot(opts.tspan, yobs_final['CPARP_total']/parp_initial, 'k');
    plt.xlim(0, 2.5e4)
