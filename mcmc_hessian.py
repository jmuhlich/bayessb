...
class MCMC(object):
    ...
    def validate(self, options):
        ...
        # logical masks for which parameters should be estimated (true) and fixed (false)
        if not len(options.estimate_params):
            raise Excepction("estimate_params must contain a list of parameters")
        options.estimate_params = np.array([p in options.model.parameters for p in options.estimate_params], dtype=bool)
        ...
        if options.sigma_adj_interval is None:
            ...
            options.sigma_adj_interval = np.floor(options.anneal_length / 10);
            
        return options
        
    def initialize(self):
        # convenience vars with lengths of rate and ic vectors
        ##self.num_rates = len(options.start_rates);
        ##self.num_species = len(options.start_ics);
        ...
        self.estimate_idx = [i for i, p in enumerate(self.model.params)
        ...
        self.T_decay = -log(1e-6) / options.anneal_length;
            
        self.ode_options = {};
        if options.reltol is not None:
            self.ode_options['reltol'] = options.reltol
        if options.abstol is not None:
            self.ode_options['abstol'] = options.abstol

        self.start_iter = 0;
        self.acceptance = 0;
        self.T = self.options.T_init;
        self.delta_posteriors = np.empty(options.nsteps)
        self.priors = np.empty(options.nsteps)
        self.likelihoods = np.empty(options.nsteps)
        self.posteriors = np.empty(options.nsteps)
        self.positions = nan(options.nsteps, self.num_estimate)
        self.alphas = np.empty(options.nsteps)
        self.sigmas = np.empty(options.nsteps)
        self.accepts = np.zeros(options.nsteps, dtype=bool)
        self.rejects = np.zeros(options.nsteps, dtype=bool)
        self.total_times = np.empty(options.nsteps)
        # FIXME only store distinct hessians -- it doesn't change on every iter
        self.hessians = np.empty((options.nsteps, self.num_estimate, self.num_estimate))
    ...
    def estimate(self):
        ...
        while self.iter <= self.options.nsteps:
            ...
            # update hessian
            if self.options.use_hessian and self.p % self.options.hessian_period == 0:
                try:
            ...
            if self.iter < self.options.anneal_length \
               and self.iter % self.options.sigma_adj_interval == 0:
                if self.acceptance / self.iter < self.options.accept_rate_target:
                    if self.sig_value > self.options.sigma_min:
                        self.sig_value -= self.options.sigma_step
                    elif self.sig_value < self.options.sigma_max:
                        self.sig_value += self.options.sigma_step
                self.T = 1 + (self.options.T_init - 1) * math.e ** (-self.p * self.T_decay);
                
            # log some interesting variables
            self.positions[self.p,:] = self.test_position
            ...
            self.sigmas[self.p] = self.sig_value
            self.ts[self.p] = self.T
            self.hessians(self.p,:,:) = self.hessian;
        ...
        def accept_move(self):
            ...
            self.acceptance += 1
            self.accepts[self.p] = 1
        
        def reject_move(self):
            self.rejects[self.p] = 1;
        
        def simulate(self, position=None):
            if not position:
                position = self.position
            # FIXME need to pass param values, extracted from position
            yout = pysb.integrate.odesolve(self.options.model, self.options.tspan)
            return yout
        
        def cur_params(self, position):
            """Return the parameter values corresponding to our current position
            in phase space."""
            # start with the original values
            rates = [p.value for p in model.parameters]
            # now "overlay" any rates we are estimating, by extracting them from
            # position and inverting the log transform
            rates[self.estimate_idx] = 10 ** position
        end
        ...
        def generate_new_position(self):
            ...
            if not self.options.use_hessian or self.iter < self.options.hessian_period:
                ...
                eig_vec, eig_val = np.linalg.eig(self.hessian)
                adj_eig_val = abs(eig_val)
                # clamp eigenvalues to a lower bound of 0.25
                adj_eig_val = np.maximum(adj_eig_val, 0.25)
                step = (eig_vec / np.sqrt(adj_eig_val)).dot(step) * 0.085
        ...
        def calculate_likelihood(self, position=None):
            if not position:
                position = self.position
            likelihood = self.options.likelihood_fn(self, position)
        end

        def calculate_posterior(self, position=None):
            if not position:
                position = self.position
            prior = self.options.prior_fn(self, position)
            likelihood = self.options.likelihood_fn(self, position)
        ...
        # http://www.mathworks.com/matlabcentral/fileexchange/11870-numerical-derivative-of-analytic-function
            if not position:
                position = self.position
            d = 0.1
            hessian = np.empty(self.num_estimate)
            ...
            for i in range(self.num_estimate):
                ...
                position_f[i] += d
                position_b[i] -= d
                f = self.calculate_posterior(position_f)
                c = self.calculate_posterior(position)
                b = self.calculate_posterior(position_b)
            ...
            for i in range(self.num_estimate-1):
                for j in range(i, self.num_estimate):
                    ...
                    position_bb[i] -= d;
                    position_bb[j] -= d;
                    ff = self.calculate_posterior(position_ff)
                    fb = self.calculate_posterior(position_fb)
                    bf = self.calculate_posterior(position_bf)
                    bb = self.calculate_posterior(position_bb)
...
class MCMCOpts(object):
    ...
    def __init__(self):
        ...
        self.estimate_params    = None    # boolean array indicating which rates to estimate
